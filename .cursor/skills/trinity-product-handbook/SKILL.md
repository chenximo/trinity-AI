...
description: >-
  ...
  管理闭环、产品地图、能力地图、产品总览、子总览、roadmap.yml、week-progress、手册规范。
...
| **能力地图（AI API · L0 产品树）** | `apps/trinity-product/docs/ai-api-platform/capability-map.md` |
| 域 L1 样板 | `user/models/index.md` · `user/identity-org/index.md` |
| L2 / 标准叶子样板 | `user/models/list.md` · `user/identity-org/workspace.md` + `user/models/roadmap.yml`（roadmap 可选附录） |
...
`产品手册` · `trinity-product` · `手册规范` · `管理闭环` · `产品地图` · `能力地图` · `产品总览` · `子总览` · `子能力` · `roadmap.yml` · `week-progress` · `ProductRoadmap` · `ProductWeekProgress`
...
4. **周计划**：全站仅 `week-progress.yml`，挂在聚合 **产品总览**；`focus` 从 **标准叶子页** 多选（`weekProgressFocusLeaves.ts`），看板可点叶子；任务写在 `plan`，风险/依赖/顺延写在 `blockers`（备注）；二者书写与看板样式相同（`①` 分行、`@姓名`、续行、`|-` 多行块）。**无「达标」列**；细则见更新规范 §四「plan 与 blockers」。
...
8. **AI API · 能力树**：补文档前先 READ `capability-map.md` 归域 A–J；新能力先改能力地图 §1–§2，再写 L1/L2；**禁止**在 product-core / 总览 / 侧栏重复粘贴整棵 A–J 树。
9. **标准叶子页（唯一叶型）**：五件套 = 说明 · 工程 · 体验/online · **`{slug}.roadmap.yml` + `<ProductRoadmap rel="…" />`**（与 `models/list` 同，localhost 可编辑；禁止 Markdown 子能力表）· 附录；禁止需求型叶、tip、手册 checklist。
