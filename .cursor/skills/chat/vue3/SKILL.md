...
1. **Store hooks 和类型统一从 `tuikit-atomicx-vue3` 引**
   ...
   - ✅ `import type { ConversationInfo } from 'tuikit-atomicx-vue3/types'`
...
src/
├── im/
│   ├── init.ts                  # 创建 chat 实例 + login（路径 A 由 _starter/login-auth.md 生成）
│   ├── events.ts                # SDK 事件订阅（消息接收 / 会话更新等）
│   └── messages/
│       ├── order.ts             # features/send-custom-message.md（业务类型: 订单 → businessID = "order"，团队有命名空间约定就跟随）
│       └── ...                  # 其他业务自定义消息（rating / coupon / red-packet / vote 等；文本发送的代码归属 MessageInput.vue 组件本身，不另起文件）
├── views/
│   └── Login.vue                # _starter/login-auth.md § 8（仅路径 A 生成；路径 B 不生成）
└── components/
    └── chat/
        ...
        └── bubbles/             # 业务卡片渲染组件，由 features/send-custom-message 命中时按需追加
...
> 见 `_starter/login-auth.md` §7 参考实现。
> 凭据获取（SDKAppID + UserSig）见 `~/.trtc-chat/knowledge-base/gen-usersig.md`（`index.html` 加 `<script>` + `window.genTestUserSig`）。
...
> 见 `_starter/message-list.md` §7 参考实现。
