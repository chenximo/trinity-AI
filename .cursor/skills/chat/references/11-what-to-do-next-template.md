...
> AI 在 A.4 或 B.5 收尾时 `read_file` 本文件，按下方模板 + 拼装规则生成 `<projectRoot>/.trtc-chat/WHAT-TO-DO-NEXT.md`。
>
...
当前固定对话对象：`{targetID}`（类型：{targetType}），写死在 `.env.local` 的 `VITE_TRTC_CHAT_TARGET_ID` 中。
