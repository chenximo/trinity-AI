...
2. Platform / 框架
3. 项目风格
4. 上次会话记忆（`<projectRoot>/.trtc-chat/state.json`，`<projectRoot>` 由 `08-state-config.md` § 8.1.1 找根算法决定，**不是** `process.cwd()`）
...
> 1. 关注 `@tencent-rtc/chat-skills` 后续版本
> 2. 提需求到 [issue 链接]
...
> 完整流程见 `_base/detect-style.md`（CSS 方案 + UI 库 + 可复用组件 + 命名约定）。
...
读 `<projectRoot>/.trtc-chat/state.json`（详见 `08-state-config.md`，特别是 § 8.1.1 找根算法——dispatcher 在本步骤前必须先跑一次找根并把结果缓存到当前会话）：
...
### 6.1 物理落盘（写入 `config.json` + `state.json`）

探测完成后，将项目画像写入 `<projectRoot>/.trtc-chat/config.json`（schema 见 `08-state-config.md`）。
...
| CSS 方案 | `css_scheme` / `ui_library` / `reusable_components` | `_base/detect-style.md` |
| 基础画像 | `integration_mode` / `platform` / `naming` / `script_style` | 本文件 § 2–4 |
| 代码进度 | `baseSlicesApplied` / `extensionSlicesApplied` / `unsupportedIntents` / `changes` | `02-path-a-script.md` A.3 后 / `03-path-b-script.md` B.5 |

**落盘时机**：
- **config.json**（CSS 方案 / UI 库 / 可复用组件）：A.1 结束后立即落盘
- **state.json**（代码进度）：A.3 + A.3.x 全部完成后统一写（见 `02-path-a-script.md` A.3 后"统一写 state.json"）
- 路径 B：B.5 写入代码进度

### 6.2 恢复流程（新对话 / 跨对话）

```
新对话开始 → read_file config.json + state.json
