/**
 * AIGC 生视频价目表 → 扁平结构（对齐生图 pricing-api-image）
 */
...
/**
 * @param {import("../data/pricing-sheet-video.mjs").SheetModel[]} sheet
 * @param {Record<string, { vendorCode: string, modelName: string, attribute?: string }>} trinityMap
 */
export function normalizeAigcVideoPricing(sheet, trinityMap = {}) {
  ...
  for (const entry of sheet) {
    for (const site of ["domestic", "international"]) {
      ...
      const priceUnit = site === "domestic" ? "元/秒" : "美元/秒";
...
/** @param {ReturnType<typeof normalizeAigcVideoPricing>} models @param {Record<string, { vendorCode: string, modelName: string, attribute?: string }>} [trinityMap] */
export function indexAigcVideoByTrinity(models, trinityMap = {}) {
...
/**
 * 按 AIGC 商务 vendorCode + modelName（+ attribute）查找，供仅刊例模型使用
 * @param {ReturnType<typeof normalizeAigcVideoPricing>} models
 * @param {"domestic"|"international"} site
 * @param {{ vendorCode: string, modelName: string, attribute?: string }|null|undefined} ref
 */
export function lookupAigcVideoByRef(models, site, ref) {
  if (!ref) return null;
  const attr = ref.attribute ?? "标准价";
  const hit =
    models.find(
  ...
  if (!hit) return null;
  return { ...hit, _mapRef: { ...ref, attribute: attr } };
}

/** @param {object|null} aigcModel @param {string} resolutionLabel @param {object|null} [mapRef] */
export function aigcVideoPriceAt(aigcModel, resolutionLabel, mapRef = null) {
  ...
  const tierRow =
    ...
    aigcModel.tiers[0];
  const hit = tierRow?.resolutions?.[resolutionLabel];
