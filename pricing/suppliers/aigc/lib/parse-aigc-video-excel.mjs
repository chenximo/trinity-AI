/**
 * 解析「AIGC价格指南（商务版报价文档）.xlsx」· AIGC生视频
 * 列：厂商 / 模型名 / 属性 · 国内 480P~4K（元/秒）· 国际 480P~4K（美元/秒）
 */
...
const DOM_COLS = [3, 4, 5, 6, 7];
const INT_COLS = [8, 9, 10, 11, 12];
 ...
 * @returns {import("../data/pricing-sheet-video.mjs").SheetModel[]}
 */
export function parseAigcVideoExcel(xlsxPath) {
  const file =
    xlsxPath ??
    path.join(__dirname, "../../../AIGC价格指南（商务版报价文档）.xlsx");
  const wb = XLSX.readFile(file);
  const ws = wb.Sheets["AIGC生视频"];
  if (!ws) throw new Error('Excel 缺少 Sheet「AIGC生视频」');
...
/** @param {ReturnType<typeof parseAigcVideoExcel>} models */
export function excelVideoModelsByKey(models) {
