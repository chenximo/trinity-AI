...
| P6b | **刊例 ⊇ 官网已公布档** | 生图：官网能力×分辨率（如可灵文生图/图生图/多图参考）Trinity 未挂 → 告警 `listing_official_coverage_gap`；生视频：已 map 的官网模型/分辨率档未上架同告警；刊例多出的 AIGC 档不视为缺官网 |
...
| P9 | **刊例双轨 V1/V2** | 对比表保留 **刊例·V1** 与 **刊例·V2**；默认 `listing_policy=v2`（官网国际站优先，否则 V1=国内÷6.5）；主结论跟生效档；转售国际≠V2 真源。详见 [刊例策略-V1-V2-国际站优先.md](./刊例策略-V1-V2-国际站优先.md) |
...
**线上刊例（本表 L4）派生口径（已拍）**：默认按 **V2**（厂商官网国际站优先；无则国内÷6.5）；对比表同时保留 V1。真源 → [刊例策略-V1-V2-国际站优先.md](./刊例策略-V1-V2-国际站优先.md)。注意：账单总览文档里「L2」= 线上刊例，与本表层级编号不同，勿混。
...
**前置条件**：第一层已锁定；线上价 **每次对比 / 巡检 / listing-compare / 草案 diff 前** `GET /v1/prices` 拉最新（统一走 `refreshOnlinePricesForCompare`）。**禁止**默认 `readOnlinePricesCache` 当「当前线上」。仅 `PRICING_SKIP_ONLINE_FETCH=1` 可读缓存，且输出须标明缓存 `fetchedAt`。

### 5.1 产出

三模态 **刊例对比校验** 主表列约定一致（见 `pipeline/lib/compare-l4-columns.mjs`）：

| 列族 | 内容 |
|------|------|
| 身份 | 原厂 modelId · Trinity ID · 显示名 · 厂商 ·（生图/生视频）档位轴 |
| 锚点 | 厂商官方价（L1） |
| 进货参照 | **AIGC国内 · AIGC国际 · TokenHub · OpenRouter**（L2；生图/生视频 OR 暂无则 `—`） |
| 被校验 | 线上刊例（L4） |
| 对比 | 各参照 `*vs官方` + 刊例结论 + 备注 |

- Excel / MD：`刊例对比校验-{生文,生图,生视频}`  
- **不进主表**：百炼、火山方舟等 **L3 转售**（见供应商分表 · `汇总-供应商vs官方`）  
- 生视频：`线上刊例 slug` 放末列；AIGC 国内 vs 国际、积分折算等辅助信息写入 **备注**
