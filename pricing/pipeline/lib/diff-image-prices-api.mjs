/**
 * 生图 prices-api：线上刊例 vs 官方+AIGC 草案（按 tierKey 对齐）
 */
 ...
 */
export function compareImagePriceEntry(onlineEntry, draftEntry) {
  const onlineTiers = parseOnlineImageTiers(onlineEntry);
  const draftTiers = draftEntry ? parseOnlineImageTiers(draftEntry) : [];
  ...
  for (const key of [...keys].sort()) {
    const on =
      findTierByKey(onlineTiers, key) ??
      onlineTiers.find((t, i) => tierKeyOf(t, i, onlineTiers.length) === key);
    const dr =
      findTierByKey(draftTiers, key) ??
...
export function compareImagePricesDocuments(onlineDoc, draftDoc) {
  const draftByModel = new Map(
    (draftDoc.data ?? []).map((e) => [String(e.model).toLowerCase(), e]),
  );
  const rows = (onlineDoc.data ?? []).map((entry) =>
    compareImagePriceEntry(entry, draftByModel.get(String(entry.model).toLowerCase()) ?? null),
  );
...
export function renderImagePricesDiffMd(doc, opts = {}) {
  const s = doc.summary;
  const draftName = opts.draftFile ?? "official-prices-api-image.json";
  const lines = [
    `# 生图刊例 diff · 线上 vs 官方+AIGC 草案`,
    "",
    `> 线上：\`prices-api.json\`（${doc.onlineFetchedAt ?? "—"} · FX ${doc.onlineFx ?? "—"}）`,
    `> 草案：\`${draftName}\`（${doc.draftGeneratedAt ?? "—"} · FX ${doc.draftFx ?? 6.5}）`,
    `> 策略：${doc.draftPolicy ?? "official_vendor_image+aigc_fallback"}`,
    "",
    "## 汇总",
    "",
    "| 指标 | 数量 |",
...
export function imagePricesDiffCsvRows(doc) {
