...
- `title`：一句话短标题（写入「标题」列，便于列表扫读；可以概括，允许省略细节）
- `summary`：**完整问题描述，禁止过度压缩**。必须保留消息里的分点、列表、字段名、筛选项、汇总指标等原文要点；用户长文粘贴需求时，`summary` 应基本保留原文结构与细节（可去 @机器人 前缀与无关寒暄），不要收成一句口号
- `reporter`：发现者姓名（见上节）
- `has_screenshot`：若相关消息 `has_image` 为 true 则为 true
- `confidence`：`high` / `medium` / `low`

写入多维表时，「问题描述」以 `summary` 为主（可与 title 合并）；细节以 `summary` 为准。
