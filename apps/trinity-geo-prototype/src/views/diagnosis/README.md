...
由 `marketing/console/diagnosis.html` 的 `<main>` 迁入 Vue，顶栏走控制台壳。

## 2. 本页规则（需求）

与 HTML 原型同屏能力；口径见手册 `apps/trinity-product/docs/geo/product-design-analysis.md` §0.6。
迁完后以本文 + 页面模板为准。对应 `diagnosis.html` 旁解读 `.md` 仍可对照，不在本目录再写一份需求长文。

## 3. 工程对齐

- 路由：`/console/…`（门户 `/trinity-geo/console/…`）· `meta.nav` = `diagnosis`
- 入口：`DiagnosisPage.vue`
- 对照：`marketing/console/diagnosis.html`
...
| 文件 | 职责 |
|------|------|
| `DiagnosisPage.vue` | 整页模板（`v-pre`，不拆子组件） |
| `README.md` | 本文 |
| 壳 `useMarketingPageScripts` | 迁入后 onMounted 执行 `marketing/js/diagnosis.js` |
...
- `marketing/console/diagnosis.md`
