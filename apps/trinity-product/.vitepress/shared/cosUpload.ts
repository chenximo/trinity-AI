...
export async function uploadImageToCos(
  ...
  if (!config) {
    throw new Error(
      "COS 未配置：在 apps/trinity-product/.env.local 填写 TRINITY_PRODUCT_COS_*，或复用 apps/trinity-docs/.env.local 的 TRINITY_DOCS_COS_*（见 env.cos.example）",
    );
  }
  ...
  try {
    ...
    throw new Error(
      "缺少 cos-nodejs-sdk-v5：在仓库根目录执行 npm install 后重启 npm run dev:trinity-product",
    );
  }
