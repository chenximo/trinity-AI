...
| 路由 | `/` |
| 优先级 | P0 |
| 目标用户 | **全球化产品**的内容 / SEO / 营销负责人（典型：**Trinity AI** API 聚合） |
...
| 字段 | 文案（V1 定稿） |
|------|----------------|
| Eyebrow | 面向全球化产品的 GEO 平台 |
| **H1** | 主流 AI 答案，一站监测 |
| **H1 渐变行** | ChatGPT 与豆包 · 同屏 SOA（`.line2` + `--grad`） |
| **Pill** | 面向全球化产品的 GEO 平台（`.hero-pill`） |
...
**客户 Logo 墙**：5–8 个占位 Logo（全球化 SaaS / API / 跨境消费等类目）；灰度，Hover 彩色。  
...
```
<title>{产品名} — 全球化产品双市场 GEO 监测 | ChatGPT 与豆包 SOA</title>
