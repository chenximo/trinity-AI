...
Trinity AI 提供统一的 HTTP API 网关：通过单一 Base URL 与 API Key 调用生文、生图、生视频等能力；账户采用 **USD 预充值 Credits**，按成功调用扣费。下文按主题汇总最常见问题；计费细则见 [计费与 Credits](./guides/billing-and-credits.md)，API 细节见 [快速入门](./quickstart.md) 与各专文。
...
4. 如需付费调用，在 [Credits](https://trinitydesk.ai/account/console#credits) 充值后再使用（余额不足时返回 **402**）。
...
请邮件 **support@trinitydesk.ai**，并附上：
...
登录 [控制台 · Credits](https://trinitydesk.ai/account/console#credits) → **Purchase Credits** → 选择支付方式并完成支付。支持 **银行卡 / Stripe Link / 微信支付 / 支付宝**（经 Stripe 托管收银）。
...
网关返回 **HTTP 402**。客户端应提示用户前往 [Credits](https://trinitydesk.ai/account/console#credits) 充值。402 与 **429**（限流）含义不同，见 [错误与调试](./reference/error-codes.md) 与 [速率与限额](./guides/rate-limits.md)。
...
3. 若 **已扣款** 但 Credits 未增加，请邮件 **support@trinitydesk.ai**，附上交易 ID 与充值时间。
...
- **条件**：该笔充值 **尚未被消费**
- **金额**：**实付全额**
- **方式**：邮件 **support@trinitydesk.ai**（须含注册邮箱、Payment History 中的交易 ID、充值时间）
...
充值记录在 Credits 页 **Payment History**；可打开 **Stripe receipt** 或 Stripe Customer Portal 下载。购前可在 Stripe 流程中更新账单地址；Invoice 能力以 Stripe 账户配置为准。
...
可以。将 SDK 的 `base_url` 指向 Trinity 的 `/v1`，`api_key` 换为您的 `xh-...` Key 即可。示例见 [快速入门](./quickstart.md) 与 [Cookbook](./cookbook/)。
...
卡号等支付敏感信息由 **Stripe** 托管；Trinity **不存储** 完整卡号。账单地址在 Stripe 收银流程中采集。
...
- **余额 / 充值记录**：[Credits](https://trinitydesk.ai/account/console#credits)
- **消耗明细**：控制台 **Activity**

### 能否设置低余额提醒？

可在控制台配置 **低余额预警阈值**（具体入口以线上为准）；触发后可能通过站内通知与邮件提醒。

### 如何删除账户？

请邮件 **support@trinitydesk.ai** 说明注销需求。未使用 Credits 的处理以 [服务条款](https://trinitydesk.ai/legal/terms) 为准。
...
- 检查 [Credits](https://trinitydesk.ai/account/console#credits) 余额
...
**以实际网关响应为准。** 反馈时请附 `X-Request-Id` 与复现步骤，发送至 **support@trinitydesk.ai**。
