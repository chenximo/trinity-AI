...
> 本文档按 [`原型设计统一.skill.md`](原型设计统一.skill.md) 约定维护，与 **当前静态实现** 对齐；对接真实 API 后的差异应在 PRD 与本文档中同步修订。  
...
> **全局视觉与版式**：[Trinity原型版式与视觉规范.md](Trinity原型版式与视觉规范.md)（含 **§1.4 字号阶梯**）。
...
*文档版本：与仓库内 Chat 静态原型同步；变更 `TrinityAI/app/chat/chat-openrouter.js` 行为时请更新 §2 表格与主路径描述。*
