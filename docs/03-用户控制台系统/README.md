...
> **风格统一真源**：[用户控制台系统-风格统一规范.md](./用户控制台系统-风格统一规范.md)（**必读**）  
...
| 文档 | 说明 |
|------|------|
| **[用户控制台系统-风格统一规范.md](./用户控制台系统-风格统一规范.md)** | 双轨定位、母版 `account`、布局/组件/按钮、AI 云接入清单、禁止事项 |
...
| **[AI云-用户控制台系统-需求理解.md](./AI云-用户控制台系统-需求理解.md)** | **AI 云产品需求真源**（IA、§5.1 账号管理定稿、其余模块 §5.2～5.5） |
| [用户控制台系统-布局与样式规范.md](./用户控制台系统-布局与样式规范.md) | OpenRouter 式壳层、`account.css`、**§4.1 列表表头/操作列** |
| 用户控制台系统 · 原型总览（待写） | 模块 ↔ `views` 对照（Trinity AI / AI 云） |
...
| 路由 | 说明 |
|------|------|
| [`/user-console-spec`](http://localhost:5173/user-console-spec) | 设计枢纽 · **用户控制台系统·规范**（条文 + 各节内联样例） |
...
**AI 云** 用户控制台系统：规划在 `apps/ai-cloud` 内按 **account 五件套** 同构接入，见统一规范 §11。

---

## 与运营后台对比（一句话）

- **用户控制台系统**：`@account` + `trinity-base` + `/design-spec` → `/user-console-spec`  
