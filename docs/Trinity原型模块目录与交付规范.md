...
> **原型交付策略**：新模块以 **`views/<模块>/` 五件套** 为**唯一工程真源**；设计稿等为输入时在模块 `README.md` 写明链接即可，**不要求**并行维护与五件套对等的独立 HTML 原型；详见 **`docs/Trinity原型形态对比与AI友好交付.md`** §0。
...
| **设计对齐** | **色板与 Token**：颜色、圆角、间距等以 **`packages/tokens`**、整站 **`trinity-base.css`** 及设计应用路由 **`/design-tokens`**（`DesignTokens.vue`）为真源，模块内 CSS 用变量承接，避免魔法数。**设计规范**：已形式化的筛选 / 搜索 / Tab / 弹窗等交互，以 **`/design-spec`**（`DesignSpec.vue`）为真源。**原子组件**：上述形式化控件 **一律从 `@trinity/ui` 引入**；包内缺件时先在 **`packages/ui`** 按规范实现并导出，再回到业务引用——**禁止**在 `views/<模块>/` 内复制规范已约定的 DOM 与类名堆叠来「假装对齐」。流程与验收顺序见 **`docs/Trinity开发枢纽与AI协作流程.md`** §4.1.5；Trinity AI 侧总纲与映射表见 **`docs/TrinityAI用户站Vue还原计划.md`**；其它 app 可在一句话内写明等价约定。 |
...
**运营后台列表页（`apps/trinity-ai-admin`）**：检索 + 表格 + 分页另遵 **[《运营后台-若依式列表规范》](../apps/trinity-ai-admin/doc/运营后台-若依式列表规范.md)**（参考 `admin-keys/KeysPage.vue`），与本文五件套并行不冲突。
...
- 在 **`docs/Trinity开发枢纽与AI协作流程.md`** 或 **`.cursor/rules`** 中用一句话写明：「二次开发说明以各模块 `README.md` 为准」，缩短新会话冷启动。  
- 与「仅五件套为工程真源」的总体策略一致时，设计输入仍以 README **外链 + 日期** 收口，见 **`docs/Trinity原型形态对比与AI友好交付.md`** §0、§5。
...
| **相关阅读** | `docs/Trinity原型形态对比与AI友好交付.md`（三代形态、仅五件套交付）；**二次开发**见本文 **§6.1**；**account 类多页精简契约**见 **§7.1**；**色板 / 规范 / `@trinity/ui` 实现链**见 **`docs/TrinityAI用户站Vue还原计划.md`** 与 **`docs/Trinity开发枢纽与AI协作流程.md`** §4.1.5。 |

---

*文档版本：v1.11（§1 增补「设计对齐（色板 / 规范 / `@trinity/ui` 原子库）」总则；§7.1 布局与视觉 README 专节、§6.1 二次开发、原型真源策略见 `Trinity原型形态对比与AI友好交付.md` §0）；以团队评审结论为准。*
