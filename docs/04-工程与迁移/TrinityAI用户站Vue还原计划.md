...
| `TrinityAI/app/chat/index.html` + `chat-openrouter.js/.css` | 对话主界面 | `/chat` | 见 `apps/trinity-ai/src/views/chat/README.md` |
...
- **执行清单与分期**：见 **`docs/04-工程与迁移/Trinity对话页Vue迁移计划.md`**（与 `views/chat/`、静态 `TrinityAI/app/chat/` 对照）。  
...
- `apps/trinity-ai/src/views/chat/README.md` — 对话模块五件套与接 API。  
- 静态期备忘（归档）：`docs/06-归档与提示词/Trinity对话页原型说明（归档）.md`。
