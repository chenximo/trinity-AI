...
> **定位**：Trinity 仓库内 **Vue 原型交付的唯一规范真源**（目录、五件套、README、视觉摘要、双仓交接）。其它文档仅补充：[`00-协作与工作流/`](../00-协作与工作流/)（门户命令、双仓操作细节）、[`Trinity版式与视觉规范.md`](./Trinity版式与视觉规范.md)（颜色字号边距全文）、[`02-后台运营管理系统设计/`](../02-后台运营管理系统设计/)（运营后台列表）、[`03-用户控制台系统/`](../03-用户控制台系统/)（租户控制台）、[`08-方法论与汇报/`](../08-方法论与汇报/)（为何五件套，可选读）。  
