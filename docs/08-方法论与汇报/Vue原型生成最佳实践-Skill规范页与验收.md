...
> **视觉验收页**：设计枢纽 `/user-console-spec`（示例：用户后台）  
...
| **触发词固定** | 任务写「按 trinity-user-console / 用户后台规范」，比只丢 URL 稳 |

**维护**：行为变更 → 改 Skill 规则表 + 规范页同 ID 行 + 样例；仅视觉微调且 Skill 已覆盖 → 可只改样例。

用户后台示例 Skill：`.cursor/skills/trinity-user-console/SKILL.md`。

**PM 提需求**：自然语言说明范围即可；Agent 读对应 Skill + `/user-console-spec` 锚点验收（用户后台见 `trinity-user-console` 附录 B）。
...
用户后台已落地示例：Skill **`trinity-user-console`** · 规范页 **`/user-console-spec`** · 文档 **`docs/03/`**。
...
| 方式 | 示例 | 说明 |
|------|------|------|
| **任务里点名** | 「按 **trinity-user-console** / 用户后台规范做××列表」 | Agent 加载对应 Skill |
| **规则引用** | `.cursor/rules/trinity-design-spec-first.mdc` 已链用户后台 Skill | 改 UI 时提醒读 token / design-spec |
| **设计枢纽入口** | `/user-console-spec`、`/design-spec` | 人审视觉；Agent 用锚点 URL |

不推荐：只发「参考 `/user-console-spec`」而不点名 Skill——Agent 可能只扫页面代码、漏检查清单。

### 3.2 Agent 读盘顺序（用户后台示例）
...
3. **叙事/状态**：`docs/03/用户控制台-布局与样式规范.md` §4.1（实施状态）  
...
用户后台示例：`docs/03/用户控制台-布局与样式规范.md` §4.1 + `README.md` 索引。
...
## 10. Agent 交付前检查清单（用户后台 · 可复用到其他域）

生成或修改 Vue 后，Agent 自检：

- [ ] 分轨正确（用户后台非 Element Plus 若依壳）  
...
| 用途 | 路径 / URL |
|------|------------|
| 用户后台 Skill | `.cursor/skills/trinity-user-console/SKILL.md` |
| 用户后台规范页 | `/user-console-spec`（`apps/trinity-design/.../UserConsoleSpecHub.vue`） |
| 主列样例锚点 | `/user-console-spec#spec-2-main` |
| 三按钮横排样例 | `/user-console-spec#spec-sample-table-actions-buttons` |
| 用户后台文档 | `docs/03/用户控制台-布局与样式规范.md` |
...
| 日期 | 说明 |
|------|------|
| 2026-05-21 | 初稿：归纳 Skill + 规范页 + docs 三层、流程、规范先行、用户后台示例与检查清单。 |
