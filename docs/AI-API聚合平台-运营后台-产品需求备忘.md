...
| ID | 需求 | 状态 | 提出 | 详设/计划 | 实现指针（原型） |
|----|------|------|------|-----------|------------------|
| B-01 | 列表 **表格**：浅灰表头、**无** 斑马纹/外边框网格（若依式横线表） | 原型已实现 | 2026-05-18 | **[运营后台-若依式列表规范.md](../apps/trinity-ai-admin/doc/运营后台-若依式列表规范.md)** | `admin-ruoyi.css`、`admin-ep-table-wrap` |
...
> **背景**：对照 [`apps/trinity-ai-admin/doc/创建后端需实现页面与功能.md`](../apps/trinity-ai-admin/doc/创建后端需实现页面与功能.md) **§12**，V1 页面/mock **已覆盖**；下列为 **文档与说明** 补强，**不阻塞** 后端开工。真源任务表：[`后台原型总览.md`](../apps/trinity-ai-admin/doc/后台原型总览.md) **§8**。
...
| [`Trinity原型版式与视觉规范.md`](./Trinity原型版式与视觉规范.md) | 全局版式与 token |
| [`产品管理随手记.md`](./产品管理随手记.md) | 原始想法；重大条目应升格到详设或本备忘 |
| [`apps/trinity-ai-admin/doc/后台原型总览.md`](../apps/trinity-ai-admin/doc/后台原型总览.md) | 模块 ↔ 目录、V1 对照结论、**§8 文档 TODO** |
